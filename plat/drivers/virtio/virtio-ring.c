							   ...
							   uint16_t num)
{
	...
	UK_ASSERT(used_descs > 0);

	for (i = head; used_descs > 0; used_descs--) {
		desc = &(vq->desc[i]);

		UK_ASSERT(vq->bufs[i].len <= MAX_BUFFER_LEN);
		 ...
		 * 'struct io_buffer'.
		 */
		UK_ASSERT(vq->bufs[i].data == (uint8_t *) &vq->bufs[i]);
...
int virtq_init_rings(struct uk_alloc *a,
		     uint16_t pci_base, struct virtq *vq, int selector)
{
	uint8_t *data;
	...
	uk_posix_memalign_ifpages(a, (void **) &data, __PAGE_SIZE,
				  VIRTQ_SIZE(vq->num));
	if (!data)
		return -ENOMEM;
...
void virtq_fini_rings(struct uk_alloc *a, struct virtq *vq)
{
	/* TODO: Experimental for now */
	uint8_t *data;

	data = (uint8_t *)vq->desc - VIRTQ_OFF_DESC(vq->num);
	uk_free(a, data);
}
